...
for (ix in 1:xseql) {
  ...
  Cspam <- nearest.dist( egdx, delta=1., diag=FALSE)
  Dspam <- nearest.dist( egdx, delta=1.5,diag=FALSE)
...
for (id in 1:dvall) {
  ...
  Cspam <- nearest.dist( egdx, delta=dval[id],diag=FALSE)
...
gridmat <- diag.spam(x^2) + .2 * nearest.dist( egdx, delta=1.,diag=FALSE) +
  .1 * nearest.dist( egdx, delta=1.5,diag=FALSE)
