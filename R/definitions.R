                     ...
                     )
{
    if(is.null(rowpointers)) {
        rowpointers <- c(1,rep_len_long(2,dimension[1]))
    }
...
"as.spam.numeric" <- function(x, eps = getOption("spam.eps")) {
    ...
    if (identical(nz,0)){
        return(
            .newSpam(
                rowpointers = c(1,rep_len_long(2,lx)),
                dimension = c(lx,1),
                force64 = force64
            )
        )
...
spam.numeric <- function(x, nrow = 1, ncol = 1, eps = getOption("spam.eps"))  {
    ...
    if (lenx != prod(nrow,  ncol)) {
        ...
        x <- rep_len_long(x, prod(nrow,ncol))
    }
...
function (x, rw, cl,value)
{
  ...
  if (is.vector(rw) && is.vector(cl)) {
    ...
    z <- .C64("subass",
              ...
              b = value,
              bj = rep_len_long(sort(as.integer(cl)),nrw*length(cl)),
              bi = bia,
...
.spam.diagaddmat <- function(x,y){
    ...
    z <- .C64("diagaddmat",
              ...
              a = c(y@entries, rep_len_long(0,minrc)),
              ja = c(y@colindices, rep_len_long(0,minrc)),
              ia = y@rowpointers,
